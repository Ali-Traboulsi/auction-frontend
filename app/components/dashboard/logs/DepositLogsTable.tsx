...
import DepositModal from "../../cards/DepositModal";
...
const DepositLogsTable: React.FC<ILogsTable> = ({ title, table_data ,handleSearchField,defaultSearchInput}) => {
  ...
  function getModalData(bid_id) {
    ...
    fetch(
      `${process.env.API_BASE_URL}/api/user/deposit/preview/${bid_id}`,
      requestOptions
    )
      .then((res) => res.json())
      .then((data) => {
        setModalData(data.data.deposit);
        setOpenModal(true);
      })
      .catch((err) => {
              ...
              table_data.map((item, index) => {
                ...
                return (
                  <tr key={index}>
                    ...
                    <td className={`${tdStyle} w-[177px]`}>
                      {item.txnid ? item.txnid : "-"}
                    </td>
                    <td className={`${tdStyle} w-[142px]`}>
                      {symbol} {c(item.amount)} <br />
                    </td>
                    ...
                    <td
                      className={`${`${tdStyle} w-[88px]`} flex items-center justify-end gap-[12px]`}
                    >
                      {/* View button  */}
                      <IconButton
                        styles={"auc-icon-btn-primary"}
                        clickHandler={() => handleModalWithId(item.txnid)}
                        svg={
                          <svg
                            className="mx-auto"
                            xmlns="http://www.w3.org/2000/svg"
